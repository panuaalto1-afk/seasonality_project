...
# -*- coding: utf-8 -*-
"""
regime_detector.py v1.2 - Market Regime Detection System

FIX v1.2 (2025-11-07):
- MOVED: regime_history.csv → seasonality_reports/regime/ (better organization)
- Added debug message when saving history

FIX v1.1 (2025-11-07):
- CRITICAL: Fixed CSV parsing - converts strings to numeric
- Skips ticker name row (yfinance CSV format)
- Same fix as auto_decider.py v4.3.3
...
class RegimeDetector:
    ...
    def _load_prices(self, ticker: str, lookback_days: int = 60) -> Optional[pd.DataFrame]:
        ...
        FIXED v1.1: Converts string columns to numeric (same fix as auto_decider.py v4.3.3)
        ...
        try:
            ...
            # CRITICAL FIX: Skip first row if it contains ticker symbols (yfinance CSV format)
            # yfinance sometimes has: Date,Open,High,Low,Close,Adj Close,Volume
            #                        ,SPY,SPY,SPY,SPY,SPY,SPY  <- Skip this row
            if 'Date' in df.columns:
                first_date = str(df.iloc[0]['Date']).strip()
                if first_date == '' or first_date == ticker.upper() or not first_date[0].isdigit():
            ...
            df = df.dropna(subset=['close'])  # Poista rivit joissa close on NaN
            
            df['date'] = pd.to_datetime(df['date'], errors='coerce')
            df = df.dropna(subset=['date'])  # Poista rivit joissa date on NaT
    ...
    def _save_to_history(self, result: Dict):
        ...
        v1.2: Siirrretty seasonality_reports/regime/ kansioon (parempi organisointi)
        """
        history_path = Path("seasonality_reports/regime/regime_history.csv")  # MUUTETTU v1.2
        ...
        print(f"[RegimeDetector] Saved to: {history_path}")  # LISÄTTY debug-viesti v1.2
...
def test_detector():
    ...
    print("\n" + "="*80)
    print("🧪 TESTING RegimeDetector v1.2 (Regime Folder)")
    print("="*80 + "\n")
    ...
    print(f"\n📁 History saved to: seasonality_reports/regime/regime_history.csv")
